...
cc=rainbow(7)
  ...
  if keyword_set(realtime) then begin
    if n_elements(data) gt 10 then begin
      ...
      if ch eq 'SPS1' then begin
        ...
        oplot, data[wgood[-11:-1]].SPS1_TIME_SINCE_ON,data[wgood[-11:-1]].SPS1_QUAD_Y, color=cc[3], thick=3  ; green
        xx = !x.crange[0] * 0.9 + !x.crange[1] * 0.1
        yy = 0.75
        dy = 0.25
        xyouts, xx, yy, 'X Rel. Pos. '+string(median(data[wgood[-11:-1]].SPS1_QUAD_X),format='(F6.3)') $
          +' +/- '+string(stddev(data[wgood[-11:-1]].SPS1_QUAD_X),format='(F7.4)'), color=cc[0], charsize=cs, charthick=ct
        xyouts, xx, yy-dy, 'Y Rel. Pos. '+string(median(data[wgood[-11:-1]].SPS1_QUAD_Y),format='(F6.3)') $
          +' +/- '+string(stddev(data[wgood[-11:-1]].SPS1_QUAD_Y),format='(F7.4)'), color=cc[3], charsize=cs, charthick=ct
        xyouts, xx, yy-dy*2, 'Quad Sum Mean '+string(median(data[wgood[-11:-1]].SPS1_QUAD_SUM),format='(F8.0)') $
          +' +/- '+string(stddev(data[wgood[-11:-1]].SPS1_QUAD_SUM),format='(F9.1)') , charsize=cs, charthick=ct
        return
      endif
    endif
    return
  ...
  if (ch eq 'SPS1') then begin
    ...
    psignal = data[-11:-1].sps1_quad_sum
    wgood = where( data.have_sps1 eq 1, numgood )
    ...
    ; sdark = min(data.sps_data)
    quadDarkFactor = [ 1.0, 1.0, 1.0, 1.0 ]
    psignal = data[-11:-1].sps2_quad_sum
  ...
  if (ch eq 'X55') then begin
    ...
    plot,plotdata[-1].x55_spectra,title='X55 Spectra',ytitle='Counts',xtitle='Bin'
  endif else begin
    ...
    if (isQuad ne 0) then begin
      ...
      ;quadx = reform(((quadsignal[2,*]+quadsignal[3,*]) - (quadsignal[0,*]+quadsignal[1,*])))/quadtotal
      ;quady = reform(((quadsignal[0,*]+quadsignal[3,*]) - (quadsignal[1,*]+quadsignal[2,*])))/quadtotal
      ...
      if (ch eq 'SPS1') then begin
        ...
        quadx = data.sps1_quad_x
        quady = data.sps1_quad_y
        doPrint = 1
      endif else if (ch eq 'SPS2') then begin
        ;  replace with TM values for Quad X and Y
        quadx = data.sps2_quad_x
        quady = data.sps2_quad_y
        doPrint = 1
      endif
    endif
    ...
    if (doPrint ne 0) then begin
      print, ' '
      print, ch, ' Median signal ', median( psignal ), ' +/- ', stddev(psignal)
      print, '     Quad X Median ', median( quadx), ' +/- ', stddev(quadx)
      print, '     Quad Y Median ', median( quady), ' +/- ', stddev(quady)
      print, ' '
    endif
    ...
    if (doSURF ne 0) then begin
      ...
      plotdata.signal_per_mA = psignal / (plotdata.surfbc > 1E-6)
    ...
    if (numgood gt 10) then begin
      ...
      if (isQuad ne 0) then begin
        ...
        if keyword_set(quad45) then begin
          ...
          oplot, ptime[-11:-1], quadx[-11:-1], color=cc[0]  ; red
          oplot, ptime[-11:-1], quady[-11:-1], color=cc[3]  ; green
          xyouts, xx, yy, 'X Rel. Pos. '+string(median(quadx),format='(F6.3)') $
            +' +/- '+string(stddev(quadx),format='(F7.4)'), color=cc[0], charsize=cs, charthick=ct
          xyouts, xx, yy-dy, 'Y Rel. Pos. '+string(median(quady),format='(F6.3)') $
            +' +/- '+string(stddev(quady),format='(F7.4)'), color=cc[3], charsize=cs, charthick=ct
          xyouts, xx, yy-dy*2, 'Quad Sum Mean '+string(median(psignal),format='(F8.0)') $
            +' +/- '+string(stddev(psignal),format='(F9.1)') , charsize=cs, charthick=ct
        endelse
      endif
    endif
  endelse
